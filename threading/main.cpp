...
TEST(Running, SingleThread) {
    ...
    Thread* thread = createAndSetThreadToRun("Multiply", multiply, (void*)testMultiply, DEFAULT_PRI);
    stopSystem();
...
TEST(Running, PriorityOrder) {
    ...
    setVerbose(true);
    int numThreads = 3;
    PriorityInfoList* infoList = (PriorityInfoList*)malloc(sizeof(PriorityInfoList));
    infoList->tcbiList = (ThreadCallbackInfo**)malloc(sizeof(ThreadCallbackInfo*)*numThreads);
    infoList->list = NULL;
    infoList->size = 0;
    ...
    *(infoList->numThreadsStarted) = 0;
    ...
    Thread* lowPriThread = createAndSetThreadToRun("Low Pri Thread", recordThreadPriority, (void*)infoList, DEFAULT_PRI-1);
    stopSystem();

    EXPECT_EQ(0, strcmp("Hi Pri Thread", infoList->list[0].threadName));
    EXPECT_EQ(DEFAULT_PRI+1, infoList->list[0].priority);
    EXPECT_EQ(0, strcmp("Mid Pri Thread", infoList->list[1].threadName));
    EXPECT_EQ(DEFAULT_PRI, infoList->list[1].priority);
    EXPECT_EQ(0, strcmp("Low Pri Thread", infoList->list[2].threadName));
...
TEST(Running, SetMyPriority) {
    ...
    *arg = MAX_PRI;
    Thread* thread = createAndSetThreadToRun("Thread", setMyPriorityTest, (void*)arg, DEFAULT_PRI);
    stopSystem();
...
TEST(Sleep, SingleThread) {
    ...
    Thread* thread = createAndSetThreadToRun("Sleep", sleepTest, (void*)sleepInfo, DEFAULT_PRI);
    stopSystem();

    EXPECT_EQ(sleepInfo->ticksToSleep, sleepInfo->tickWokenUp-sleepInfo->tickSleepStarted);
...
TEST(Sleep, MultipleThreads) {
    ...
    Thread* thread = createAndSetThreadToRun("Sleep", sleepTest, (void*)&sleepInfo[0], DEFAULT_PRI);
    Thread* thread2 = createAndSetThreadToRun("Sleep", sleepTest, (void*)&sleepInfo[1], DEFAULT_PRI);
    stopSystem();

    for (int x=0; x <2; x++) {
        EXPECT_EQ(sleepInfo[x].ticksToSleep, sleepInfo[x].tickWokenUp-sleepInfo[x].tickSleepStarted);
    }
...
//    setVerbose(true);
    ...
    Thread* thread = createAndSetThreadToRun("Lock", simpleLock, (void*)threadHoldingLock, DEFAULT_PRI);
    stopSystem();

    if(threadHoldingLock->name) EXPECT_EQ(0, strcmp(thread->name, threadHoldingLock->name));
...
TEST(Locking, PriorityDonation) {
    ...
    *numThreadsFinished = 0;
    ...
    ThreadLockInfo* threadOrder = (ThreadLockInfo*)calloc(numThreads, sizeof(ThreadLockInfo)*numThreads);
    const char* lock =  createLock();
    bool* cont = (bool*)malloc(sizeof(bool));
    *cont = false;
    DonationInfo* donationInfo = (DonationInfo*)malloc(sizeof(DonationInfo)*numThreads);
    ...
    Thread* lowPriThread = createAndSetThreadToRun("Low Pri Thread", donationPriority, (void*)&donationInfo[0], DEFAULT_PRI);

    stopSystem();

    EXPECT_EQ(DEFAULT_PRI+2, donationInfo->threadOrder[0].thread.priority);
    EXPECT_EQ(0,strcmp("Low Pri Thread",donationInfo->threadOrder[0].thread.name));
